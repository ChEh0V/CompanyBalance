window.onload = function() {
    ...
    // ИНДИВИДУАЛЬНАЯ ОПЕРАЦИЯ (Тема: Услуги → Добавочный капитал)
    // При нажатии на кнопку "Balance Sheet" текущее число на экране трактуется как выручка от услуг (руб.)
    // Рассчитывается рекомендуемый добавочный капитал, результат выводится в поле калькулятора (без alert)
    document.getElementById("btn_foundation").onclick = function() {
        ...
        if (isNaN(revenue) || revenue <= 0) {
            outputElement.innerHTML = "Ошибка: введите >0";
            return;
        }
        ...
        // Вывод результата в поле калькулятора (без alert)
        if (!selectedOperation) {
